...
impl TursoDriver {
    pub async fn new_pool(url: &str, auth_token: &str) -> Result<Self> {
    ...
    pub async fn ping(url: &str, auth_token: &str) -> Result<()> {
        let db = Builder::new_remote(url.to_string(), auth_token.to_string())
            .build()
            .await?;
        let conn = db.connect()?;
    ...
    pub async fn get_materialized_views(&self) -> Result<Vec<String>> {
        // libSQL/Turso does not support materialized views.
        return Ok(Vec::new());
    }
    ...
    pub async fn query(
        ...
        let mut rows = self.conn.query(&sql, params![limit as i64, offset as i64]).await?;
