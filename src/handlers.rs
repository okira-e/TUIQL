...
use crate::{
    actions::{Action, AppAction, DbAction, ExplorerAction, JsonViewAction, ResultsTableAction},
    app::{App, View},
};
...
impl App {
    ...
    pub fn handle_key_event(&mut self, key: KeyEvent) {
        ...
        let action = match (key.modifiers, key.code) {
            (KeyModifiers::CONTROL, KeyCode::Char('c')) => Action::App(AppAction::Quit),
            (_, KeyCode::Tab) => Action::App(AppAction::CyclePane),
            (_, KeyCode::Esc) => Action::App(AppAction::ClosePopup),
            // View specific
            _ => match self.focused_view {
                View::ResultsTable => match (key.modifiers, key.code) {
                    ...
                    (_, KeyCode::Char('g')) => Action::ResultsTable(ResultsTableAction::GoToFirst),
                    (_, KeyCode::Char('G')) => Action::ResultsTable(ResultsTableAction::GoToLast),
                    (_, KeyCode::Char('n')) => Action::Db(DbAction::NextPage),
                    (_, KeyCode::Char('p')) => Action::Db(DbAction::PrevPage),
                    (_, KeyCode::Enter) => Action::App(AppAction::ViewSelectedRowAsJson),
                    (_, KeyCode::Char('y')) => {
                ...
                View::JsonView => match (key.modifiers, key.code) {
                    ...
                    (_, KeyCode::Char('g')) => Action::JsonView(JsonViewAction::GoToFirst),
                    (_, KeyCode::Char('G')) => Action::JsonView(JsonViewAction::GoToFirst),
                    (_, KeyCode::Char('y')) => {
                        Action::ResultsTable(ResultsTableAction::YankSelection)
                    }
